...
class VideoProcessor
{
    ...
    public function upload($videoUploadData)
    {
        ...
        if (move_uploaded_file($videoData["tmp_name"], $tempFilePath)) {
            ...
            if (!$this->generateThumbnail($finalFilePath)) {
                echo "upload  failed  -cant generate thumbnail 2\n";
                return false;
            }
    ...
    private function insertVideoData($uploadData, $filePath)
    {
        ...
        $query = $this->con->prepare("INSERT INTO videos(uploadedBy, title, description, privacy, filePath, category,duration)
                                        VALUES(:uploadedBy, :title, :description, :privacy, :filePath, :category,:duration)");
        ...
        if (!$result) print_r($query->errorInfo());
        return $result;
    }
